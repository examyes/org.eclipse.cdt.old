...
public class GdbLocalVariablesMonitorManager extends LocalVariablesMonitorManager
{
   ...
   public void updateLocalMonitors()
   {
	...
	if (Gdb.traceLogger.EVT)
		Gdb.traceLogger.evt(
			2,
			"GdbLocalVariablesMonitorManager.updateLocalMonitors is updating local variables...");
	...
	try {
		for (i = 0; i < _threads.size(); i++) {
			...
			if (tc == null) {
				...
				removeLocalVariablesMonitor(
					stackInfo.getDU(),
					stackInfo.getStackEntryNum(),
					false);
				//HC
				continue;
			}
			...
			if (Gdb.traceLogger.DBG)
				Gdb.traceLogger.dbg(
					2,
					"GdbLocalVariablesMonitorManager.updateLocalMonitor #### GETTING LOCAL_VARIABLES for gdbTID="
						+ gdbTID);
			...
			if (m == null) {
				if (Gdb.traceLogger.ERR)
					Gdb.traceLogger.err(
						2,
						"######## GdbLocalVariablesMonitorManager.updateLocalMonitor moduleID="
							+ moduleID
							+ " module==NULL,  BYPASSING");
				return;
			}
				...
				for (j = 0; j < locals.length; j++) {
					...
					if (x > 0) {
						...
						if (Gdb.traceLogger.DBG)
							Gdb.traceLogger.dbg(
								2,
								"GdbLocalVariablesMonitorManager.updateLocalMonitor varName="
									+ varName
									+ " varValue="
									+ varValue);
					}
...
//						monVar.setScalarValue(varValue);
					...
					else
					{
						...
						Gdb_VariableMonitor vm = vmm.getMonitor(id.intValue());
						GdbVariable monVar = (GdbVariable)vm.getMonitoredVariable();
						
						if (!monVar.getScalarValue().equals(varValue))
						{						
							exprID = id;
		                    monVar.setScalarValue(varValue);
		                    vm.updateVariable();
		                     ...
		                     vmm._changedMonitors.addElement(vm);

							if (Gdb.traceLogger.DBG)
								Gdb.traceLogger.dbg(
									1,
									"Updating local variable " + varName + " exprID=" + exprID);
						}
					}
				}
