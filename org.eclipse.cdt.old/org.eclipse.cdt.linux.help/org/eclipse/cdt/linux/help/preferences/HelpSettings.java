...
public class HelpSettings
{
	...
	ArrayList list= getPreferences();
	CppPlugin.writeProperty("Help",list);
	...
	//set default properties (only for remote projects)
	...
	for (int i=0; i<list.size(); i++)
		...
		if (name.equals(IHelpSearchConstants.HELP_SEARCH_TYPE_EXACT) ||
			...
			putPreference(name,b);
		    }
		else
		    {
			//strings
			if(value.equals("null"))
			    putPreference(name,null);
			else
