...
public class OpenEditorAction extends Action implements IOpenAction
{
	...
	public DataElement getResourceFor(DataElement element)
	{
		...
		String fileName = element.getAttribute(DE.A_SOURCE);
		DataElement resource =
			_api.findResourceElement(element.getDataStore(), fileName);
		...
		if (_element != null)
		{
			...
			if (_element.getParent() != dataStore.getDescriptorRoot())
			{
				...
				DataElement projectElement =
					_api.getProjectFor(resourceElement);
				...
				String fileName =
					(String) (resourceElement
						.getElementProperty(DE.P_SOURCE_NAME));
				...
				if ((fileName != null) && (fileName.length() > 0))
				{
					...
					if (!openEditor)
					{
						// if the file is not open in editor
						// don't do anything
						file = _api.findFileInEditor(fileName);
						if (file == null)
						{
							return;	
						}
					...
					if (file != null)
					{
						...
						if (_plugin != null)
						{
							...
							Integer lineLocation =
								(Integer) (_element
									.getElementProperty(DE.P_SOURCE_LOCATION));
...
protected void gotoLocation(IEditorPart editor, IFile file, int line)
{
	...
	{
		((org.eclipse.cdt.cpp.ui.internal.editor.CppEditor) editor).gotoLine(
			line);
	} else
	{
		try
