...
public class ModelInterface implements IDomainListener, IResourceChangeListener
{
	private class OpenProjectAction extends Thread
	{
		...
		public void run()
		{
			...
			{
				projectMinerProject =
					findProjectElement(_project, "Closed Project");
				...
				{
					projectMinerProject.setAttribute(
						DE.A_SOURCE,
						_project.getLocation().toString());
				}
			}
			...
			if (projectMinerProject != null)
			{
				DataElement oDescriptor =
					dataStore.localDescriptorQuery(
						projectMinerProject.getDescriptor(),
						"C_OPEN",
						4);
				if (oDescriptor != null)
				{
					// open closed project
					dataStore.synchronizedCommand(
						oDescriptor,
						projectMinerProject);
				...
				if (_project instanceof Repository)
				{
					...
					if (localWorkspace != null && projectMinerProject != null)
					{
						...
						DataElement localRemoteProject =
							((Repository) _project).getClosedElement();
						if (localRemoteProject != null)
						{
							localDataStore.deleteObject(
								localWorkspace,
								localRemoteProject);
							((Repository) _project).setClosedElement(null);
						}

						localDataStore.createReference(
							localWorkspace,
							projectMinerProject);
				...
				notifier.fireProjectChanged(
					new CppProjectEvent(CppProjectEvent.OPEN, _project));
			}
		}
	...
	public class SearchContextMenuContributor
		implements IContextMenuContributor
	{
			...
			IInputSelectionProvider inputProvider)
		{
			...
			if (results instanceof java.util.List)
			{
				...
				{
					IMarker marker =
						((ISearchResultViewEntry) input).getSelectedMarker();
					try
					{
		...
		public void show(IMarker marker)
		{
			...
			if (resource == null || !(resource instanceof IFile))
			{
				try
				{
					...
					{
						IOpenAction openA =
							CppActionLoader.getInstance().getOpenAction();
	...
	public class CreateSearchMarkersAction implements Runnable
	{
		...
		public void run()
		{
			...
			{
				DataElement output =
					((DataElement) _results.get(i)).dereference();

				String fileName =
					(String) (output.getElementProperty(DE.P_SOURCE_NAME));
				Integer location =
					(Integer) (output.getElementProperty(DE.P_SOURCE_LOCATION));
				...
				if (file == null)
				{
					DataElement fileElement =
						output.getDataStore().createObject(
							null,
							"file",
							fileName,
							fileName);
					file =
						new FileResourceElement(
							fileElement,
							(IProject) CppPlugin.getCurrentProject());
					addNewFile(file);
				}
				if (file != null)
				{
					try
					{
						...
						if (file instanceof FileResourceElement)
						{
							...
							searchMarker =
								workspaceRoot.createMarker(
									SearchUI.SEARCH_MARKER);
						} else
						{
							searchMarker =
	...
	public void monitorStatus(DataElement status)
	{
		if (status != null)
		{
			if (!_statuses.contains(status))
			{
				...
				MonitorStatusThread monitor =
					new MonitorStatusThread(status, project);
	...
	public void debug(String pathStr, String port, String key)
	{
		...
		{
			dirObject =
				dataStore.createObject(null, "directory", pathStr, pathStr);
		} else
		{
	...
	public void debug(DataElement dirObject, String port, String key)
	{
		...
		DataElement hostObj =
			dataStore.createObject(null, "hostname", hostName);
		...
		DataElement debugDescriptor =
			dataStore.localDescriptorQuery(
				dirObject.getDescriptor(),
				"C_DEBUG");
		...
		if (debugDescriptor != null)
		{
			...
			if (_plugin.getDataStore() == dataStore)
			{
				...
				DataElement javaPath =
					dataStore.createObject(null, "directory", jrePath);
	...
	public DataElement invoke(
		String path,
		String invocation,
		boolean showProgress)
	{
		// first try to find the path element
		DataElement pathElement =
			findResourceElement(findWorkspaceElement(), path);
		if (pathElement == null)
		{
			pathElement =
				_plugin.getCurrentDataStore().createObject(
					null,
					"directory",
					path,
					path);
		...
		boolean showProgress)
	{
		if (invocation != null)
		{
			...
			DataElement invocationObj =
				dataStore.createObject(null, "invocation", invocation, "");
			...
			DataElement commandDescriptor =
				dataStore.localDescriptorQuery(
					pathElement.getDescriptor(),
					"C_COMMAND");
			DataElement status =
				dataStore.command(commandDescriptor, args, pathElement, true);
			if (status != null)
	...
	public boolean isBeingEdited(IResource resource)
	{
		...
		for (int a = 0; a < windows.length; a++)
		{
			...
			for (int b = 0; b < pages.length; b++)
			{
				...
				{
					IEditorReference editorRef = editors[c];
					...
					if (editor != null && editor.getEditorInput() instanceof IFileEditorInput)
					{
						...
						if (input != null)
						{
							...
							if (file
								.getLocation()
								.toString()
								.equals(resource.getLocation().toString()))
							{
								return true;
							}
						}
					...
					if (editor != null && editor.getEditorInput() instanceof IFileEditorInput)
					{
						...
						if (input != null)
						{
									...
									== project.getLocation())
							{
								Display d = getDummyShell().getDisplay();
								d.asyncExec(
									new CloseEditorAction(page, editor));
							}
						}
					}
				}
	...
	public IFile findFileInEditor(String fileName)
	{
		IWorkbench desktop = CppPlugin.getDefault().getWorkbench();
		IWorkbenchWindow[] windows = desktop.getWorkbenchWindows();
		for (int a = 0; a < windows.length; a++)
				...
				for (int c = 0; c < editors.length; c++)
				{
					...
					if (editor != null && editor.getEditorInput() instanceof IFileEditorInput)
					{
						...
						if (input != null)
						{
							...
							if (compareFileNames(file.getLocation().toString(),fileName))
							{
								return file;	
							}
						}
	...
	public void shutdown()
	{
		...
		try
		{
			SearchPlugin.getWorkspace().getRoot().deleteMarkers(
				SearchUI.SEARCH_MARKER,
				true,
	...
	public void closeProject(IProject project)
	{
		...
		// close project
		DataElement workspace = findWorkspaceElement(dataStore);
		DataElement cprojectObj =
			dataStore.find(workspace, DE.A_NAME, project.getName(), 1);

		if (cprojectObj != null)
		{
			DataElement closeD =
				dataStore.localDescriptorQuery(
					cprojectObj.getDescriptor(),
					"C_CLOSE_PROJECT");
	...
	public void setDistributionExtensions(
		IProject project,
		String init_set_Key)
	{
		DataStore dataStore = _plugin.getDataStore();
		...
		DataElement extensionElement =
			dataStore.createObject(
				null,
				"ExtraDist File Extensions",
				project.getName());
		ArrayList extensions =
			_plugin.readProperty(project, "Extra_Dist_Extensions");
		...
		setDistributionExtensions(
			findProjectElement(project),
			extensionElement,
			init_set_Key);
	}
	public void setDistributionExtensions(
		DataElement theObject,
		DataElement theExtensions,
		String init_set_Key)
	{
		...
		if (init_set_Key.equals("init"))
		{
			setD =
				dataStore.localDescriptorQuery(
					contObj,
					"C_INIT_EXTRA_DIST_EXTENSIONS");
		} else
		{
			setD =
				dataStore.localDescriptorQuery(
					contObj,
					"C_SET_EXTRA_DIST_EXTENSIONS");
		}
		if (setD != null)
		{
	...
	public void setPaths(IProject project)
	{
		...
		{
			// include paths
			DataElement includeElement =
				dataStore.createObject(null, "path", "Include Path");
			ArrayList includePaths =
				_plugin.readProperty(project, "Include Path");
			for (int i = 0; i < includePaths.size(); i++)
			{
				dataStore.createObject(
					includeElement,
					"path",
					(String) includePaths.get(i),
					(String) includePaths.get(i));
			}

			// external source paths
			DataElement externalElement =
				dataStore.createObject(null, "path", "External Source");
			ArrayList externalPaths =
				_plugin.readProperty(project, "External Source");
			for (int i = 0; i < externalPaths.size(); i++)
			{
				dataStore.createObject(
					externalElement,
					"path",
					(String) externalPaths.get(i),
					(String) externalPaths.get(i));
			}

			// libraries
			DataElement libraryElement =
				dataStore.createObject(null, "path", "Libraries");
	...
	public void setParseIncludePath(IProject project)
	{
		...
		{
			DataElement includeElement =
				dataStore.createObject(null, "environment", "Include Path");
			ArrayList includePaths =
				_plugin.readProperty(project, "Include Path");
			for (int i = 0; i < includePaths.size(); i++)
				dataStore.createObject(
	...
	public void setParseQuality(IProject project)
	{
		...
		if (projectObj != null)
		{
			...
			// parse quality
			DataElement qualityElement =
				dataStore.createObject(null, "quality", "2");
			...
			// parse behaviour
			DataElement autoParseElement =
				dataStore.createObject(null, "autoparse", "No");
			...
			DataElement autoPersistElement =
				dataStore.createObject(null, "autopersist", "No");
			ArrayList autoPersist =
				_plugin.readProperty(project, "AutoPersist");
	...
	public DataElement findWorkspaceElement(DataStore dataStore)
	{
		...
		if (_workspaceElement != null
			&& _workspaceElement.getDataStore() == dataStore)
		{
		...
		//if (_workspaceElement == null)
		{
			...
			DataElement projectInfo =
				dataStore.findMinerInformation(
					"org.eclipse.cdt.cpp.miners.project.ProjectMiner");
			if (projectInfo == null)
			{
			...
			{
				workspaceObj =
					dataStore.find(projectInfo, DE.A_TYPE, "Workspace", 1);
	...
	public IResource findResource(DataElement resourceElement)
	{
		...
		if (type.equals("Project") || type.equals("Closed Project"))
		{
			...
			resourceElement.isOfType("file")
				|| resourceElement.isOfType("directory"))
		{
						...
						if (projectElement.getName().equals(project.getName()))
						{
							if (project.isOpen())
							{
								DataStore rdataStore =
									((Repository) project).getDataStore();
								if (rdataStore
									== projectElement.getDataStore())
								{
									return project;
								}
							} else
							{
								DataElement root =
									((Repository) project).getClosedElement();
	...
	public DataElement findOrCreateResourceElement(IResource resource)
	{
		...
		if (result == null)
		{
			...
			DataElement projectElement =
				findProjectElement(resource.getProject());
			if (projectElement != null)
			{
	...
	public DataElement findResourceElement(DataElement root, String path)
	{
		...
		} else
		{
			if (root.getType().equals("Workspace")
				|| path.startsWith(root.getSource()))
			{
		...
		boolean respectCase)
	{
		...
		{
			ArrayList args = new ArrayList();
			DataElement patternElement =
				dataStore.createObject(null, "pattern", pattern);

			patternElement.setAttribute(
				DE.A_VALUE,
				respectCase ? "check_case" : "ignore_case");
			...
			for (int i = 0; i < types.size(); i++)
			{
				...
				DataElement objDescriptor =
					dataStore.findObjectDescriptor(type);
...

			{
				...
				{
					StringBuffer patternStr =
						new StringBuffer("Pattern: " + pattern + " Types: ");
					for (int i = 0; i < types.size(); i++)
					{
					...
					_searchResultsView.searchStarted(
						...
						patternStr.toString(),
						patternStr.toString(),
						CppPlugin.getDefault().getImageDescriptor(
							"details.gif"),
			...
			DataElement status =
				dataStore.command(searchDescriptor, args, subject, true);
			_status = status;
	...
	public void cancel(DataElement command)
	{
		...
		DataElement cancelDescriptor =
			dataStore.localDescriptorQuery(cmdDescriptor, "C_CANCEL");
	...
	public IResource findFile(DataElement fileElement)
	{
		...
		} else
		{
			return findFile(
				...
				!fileElement.getDataStore().isVirtual());
		}

		return null;
	}

	public IResource findFile(
		IContainer root,
		DataElement fileElement,
		ArrayList segments)
	{
		DataElement currentSegment =
			(DataElement) segments.get(segments.size() - 1);
	...
	public IResource findFile(java.io.File fileName)
	{
		...
		if (fileName != null)
		{
			...
			if (file == null)
			{
				for (int i = 0; i < _tempFiles.size(); i++)
				{
					ResourceElement tempFile =
						(ResourceElement) _tempFiles.get(i);
					String tempFileName =
						tempFile.getLocalLocation().toString();
...

			{
				...
				if (resources != null)
				{
					...
					for (int i = 0; i < resources.length; i++)
					{
						...
						{
							FileResourceElement res =
								(FileResourceElement) resource;
						...
						if (fileNameString.startsWith(path)
							&& resource instanceof IContainer)
							//**if (resource instanceof IContainer)
						{
							IResource result =
								findFile((IContainer) resource, fileName);
							if (result != null)
								return result;
						}
					}
	...
	public DataElement findParseFiles(DataElement theProjectFile)
	{
		...
		{
			ArrayList parseRef =
				theProjectFile.getAssociated("Parse Reference");
		...
		{
			DataElement projectObjects =
				dataStore.find(parseProject, DE.A_NAME, "Project Objects", 1);
	...
	{
		String projectFileSource1 =
			theProjectFile.getSource().replace('\\', '/');
		String projectFileSource2 =
			theProjectFile.getSource().replace('/', '\\');
		DataStore dataStore = theProjectFile.getDataStore();

		while (!(theProjectFile = theProjectFile.getParent())
			.getType()
			.equals("Project"))
		{
		}

		DataElement parseProject =
			((DataElement) (theProjectFile
				.getAssociated("Parse Reference")
				.get(0)))
				.dereference();
		DataElement parsedFiles =
			dataStore.find(parseProject, DE.A_NAME, "Parsed Files", 1);
		DataElement theParseFile =
			dataStore.find(parsedFiles, DE.A_SOURCE, projectFileSource1, 1);
		if (theParseFile == null)
		{
			theParseFile =
				dataStore.find(parsedFiles, DE.A_SOURCE, projectFileSource2, 1);
		}

		if (theParseFile == null)
		{
			DataElement dummyInput =
				dataStore.find(
			...
			if (dummyInput != null)
			{
				...
				DataElement theMessage =
					((DataElement) (theParseFile.getNestedData().get(0)));
				theMessage.setAttribute(
					DE.A_VALUE,
					projectFileSource1 + " has not been parsed.");
	...
	private void synchronizeWithNavigator(
		IResource resource,
		DataElement parent)
	{
		if (resource instanceof IContainer)
...
/* this causes a performance problem at startup- we should find a better way to do this
				...
				try
				{
					...
					for (int i = 0;(i < members.length) && !needsRefresh; i++)
					{
						...
						if (!name.startsWith("."))
						{
							...
							DataElement match =
								dataStore.find(parent, DE.A_NAME, name, 1);
							needsRefresh =
								((match == null) || match.isDeleted());
						}
					}
				} catch (CoreException e)
				{
	...
	public synchronized void domainChanged(DomainEvent ev)
	{
		...
		if ((object != null) && (object.getType().equals("status")))
		{
			...
			String commandName =
				(String) commandInstance.getElementProperty(DE.P_NAME);
			...
			if (state.equals("done"))
			{
				...
				if (children != null)
				{
					...
					{
						if (commandName.equals("C_SEARCH")
							|| commandName.equals("C_SEARCH_REGEX"))
						{
							{
								Display d = getDummyShell().getDisplay();
								d.asyncExec(
									new CreateSearchMarkersAction(children));
								if (_searchResultsView != null)
								{
						...
						} else if (commandName.equals("C_COMMAND"))
						{
							...
							{
								DataElement output =
									(DataElement) children.get(i);
								{
									if (output
										.getDataStore()
										.filter(_markersDescriptor, output))
									{
										String type =
											(String) (output
												.getElementProperty(DE.P_TYPE));
										int priority;
	...
	private void removeParseInfo(DataElement element)
	{
		...
		if (element.isOfType("file") || element.isOfType("directory"))
		{
			DataElement notifyD =
				dataStore.localDescriptorQuery(
					dataStore.getRoot().getDescriptor(),
					"C_NOTIFICATION",
					1);
			if (notifyD != null)
			{
				...
				args.add(element);
				DataElement delD =
					dataStore.createObject(null, "dummy command", "C_DELETE");
		...
		} else
		{
			...
			if (parsedFile != null)
			{
				...
				if (rmParseInfo != null)
				{
					...
					DataElement status =
						dataStore.command(rmParseInfo, args, parsedFile);
					monitorStatus(status);
				}
			}
		}
	...
	private void parseFile(DataElement element)
	{
		...
		if (notifyD != null)
		{
			...
			args.add(element);
			DataElement addD =
				dataStore.createObject(null, "dummy command", "C_ADD");
	...
	private void resourceChanged(IResource resource)
	{
		if (resource != null)
		{
			...
			if (resourceElement != null)
			{
				...
				DataElement refreshD =
					dataStore.localDescriptorQuery(
						resourceElement.getDescriptor(),
						"C_REFRESH");
				if (refreshD != null)
				{
				...
				case IResourceDelta.CHANGED :
					...
					} else
					{
						...
						if (aff.length >= 2)
						{
							...
							{
								// this is a rename
								if ((oldRes instanceof IResource)
									&& (newRes instanceof IResource))
								{
									DataElement element =
										findResourceElement((IResource) oldRes);
									if (element != null)
									{
										DataStore dataStore =
											element.getDataStore();

										String oldName = element.getName();
										String oldSource = element.getSource();

										String newName =
											newRes.getLocation().toString();
				...
				case IResourceDelta.REMOVED :
					{
						...
						{
							DataElement resourceElement =
								findResourceElement(resource);
			...
			case IResourceChangeEvent.PRE_CLOSE :
				{
					...
					if (resource instanceof IProject)
					{
						...
						_projectNotifier.fireProjectChanged(
							new CppProjectEvent(
								CppProjectEvent.CLOSE,
								(IProject) resource));
					}
			...
			case IResourceChangeEvent.PRE_DELETE :
				{
					...
					if (resource instanceof IProject)
					{
						...
						DataElement cProject =
							findProjectElement((IProject) resource);
						if (cProject != null)
						{
							DataElement commandDescriptor =
								dataStore.localDescriptorQuery(
									cProject.getDescriptor(),
									"C_DELETE_PROJECT");
							if (commandDescriptor != null)
							{
	...
	{
		_projectNotifier.fireProjectChanged(
			new CppProjectEvent(event, project));
	}
