...
//
// com/ibm/debug/internal/pdt/ui/dialogs/AddressBPWizard.java, eclipse, eclipse-dev, 20011129
...
public class AddressBPWizard extends BreakpointWizard {

	protected static final String DIALOG = "AddressBPWizard.dialog";
	protected static final String PAGE_1 = "AddressBPWizard.page1";
	...
	/**
	 * Constructor for AddressBPWizard
	 */
	public AddressBPWizard() {
		super();
		initializeWizard();

	}

	public AddressBPWizard(IMarker breakpoint)
	{
		super();
		initializeWizard();
	...
	private void initializeWizard()
	{
		...
		//see if a subsection already exists
	...
	public void addPages()
	{
		...
		if(!editing)
			wizardPage = new AddressBPWizardPage(PICLUtils.getResourceString(PAGE_1+".pageName"),
				 PICLUtils.getResourceString(PAGE_1+".title"), null, deferredSupported());
		else wizardPage = new AddressBPWizardPage(PICLUtils.getResourceString(PAGE_1+".pageName"),
	...
	public boolean performFinish() {
		AddressBPWizardPage page = 	(AddressBPWizardPage)getStartingPage();
		IStatus created = createMarker(page.getProjectResource(),
				page.getAddress(), page.isDeferred());
		if(!created.isOK())
		{
	...
	private IStatus createMarker(IResource resource, String address, Boolean defer)
	{
		...
		try{
			...
			if(!editing){
				breakpointMarker = resource.createMarker(IPICLDebugConstants.PICL_ADDRESS_BREAKPOINT);
			...
			//     see the IPICLDebugConstants.PICL_ADDRESS_BREAKPOINT_ATTRIBUTES for an array of attribute names
			//     that are valid on an address breakpoint.
			...
			String[] attributeNames = {IPICLDebugConstants.UPDATE_BREAKPOINT, IPICLDebugConstants.EDITABLE, IPICLDebugConstants.DEFERRED, IPICLDebugConstants.THREAD,
				IPICLDebugConstants.EVERY_VALUE, IPICLDebugConstants.TO_VALUE,IPICLDebugConstants.FROM_VALUE,
				IPICLDebugConstants.CONDITIONAL_EXPRESSION, IPICLDebugConstants.ADDRESS_EXPRESSION,
			...
			Object[] values = {new Boolean(editing), new Boolean(true), defer, conditionalPage.getThreadValue(),conditionalPage.getEveryValue(),
				conditionalPage.getToValue(), conditionalPage.getFromValue(),conditionalPage.getExpression(), address,target.getUniqueID()};
