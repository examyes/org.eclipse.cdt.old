package com.ibm.debug.epdc;

////////////////////////////////////////////////////////////////////////////////
//
// Copyright (c) 1997, 2001, 2002 International Business Machines Corporation. All rights reserved.
// This program and the accompanying materials are made available under the terms of
// the Common Public License which accompanies this distribution.
//
// com/ibm/debug/epdc/EReqExpressionValueModify.java, java-epdc, eclipse-dev, 20011128
// Version 1.5.1.2 (last modified 11/28/01 16:24:14)
////////////////////////////////////////////////////////////////////////////////

/**
 * Note: This class/interface is part of an interim API that is still under
 * development and expected to change significantly before reaching stability.
 * It is being made available at this early stage to solicit feedback from
 * pioneering adopters on the understanding that any code that uses this API
 * will almost certainly be broken (repeatedly) as the API evolves.
 */

import java.io.*;

/**
 * Expression value modify reqpest
 */
public class EReqExpressionValueModify extends EPDC_Request {

   /**
    * Decodes request from an input buffer
    * @exception IOException if an I/O error occurs
    */
   EReqExpressionValueModify(byte[] inBuffer) throws IOException {
      super(inBuffer);

      _exprID = readShort();
      _exprTreeNode = readInt();
      _newNodeValue = readOffset();
   }

   public EReqExpressionValueModify(short id, int treeNodeId, String value)
   {
     super(EPDC.Remote_ExpressionValueModify);

     _exprID = id;
     _exprTreeNode = treeNodeId;
     _ReqNewNodeValue = new EStdString(value);
   }

   /**
    * Return expression ID
    */
   public short exprID() {
      return _exprID;
   }

   /**
    * Return expression tree node id
    */
   public int exprTreeNode() {
      return _exprTreeNode;
   }

   /**
    * Return new node value
    */
   public String reqNewNodeValue() throws IOException {
      if (_ReqNewNodeValue == null && (_newNodeValue != 0)) {
         posBuffer(_newNodeValue);
         _ReqNewNodeValue = readStdString();
      }
      return _ReqNewNodeValue.string();
   }

   public void output(DataOutputStream dataOutputStream)
   throws IOException
   {
     super.output(dataOutputStream);

     dataOutputStream.writeShort(_exprID);
     dataOutputStream.writeInt(_exprTreeNode);

     int offset = fixedLen() + super.varLen();
     writeOffsetOrZero(dataOutputStream, offset, _ReqNewNodeValue);

     if (_ReqNewNodeValue != null)
         _ReqNewNodeValue.output(dataOutputStream);

   }

  /**
   * Return the length of the fixed component
   */

   protected int fixedLen()
   {
      return _fixed_length + super.fixedLen();
   }

  /**
   * Return the length of the variable component
   */

   protected int varLen()
   {
      return totalBytes(_ReqNewNodeValue);
   }

   // data fields
   private short _exprID;
   private int _exprTreeNode;
   private int _newNodeValue;
   private EStdString _ReqNewNodeValue;

   private static final int _fixed_length = 10;

   public final static String jIBMCopyright = "(c) Copyright IBM Corporation 1997, 1998 - All Rights Reserved";

}

